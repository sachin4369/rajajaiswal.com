...
import { fetchAremrestPPBase } from '@/lib/api';
import { Product } from '@/types';

export default function AremrestPPBaseSubcategoryPage() {
  ...
  useEffect(() => {
    async function loadProducts() {
      ...
      setError(null);
      try {
        const fetchedProducts = await fetchAremrestPPBase();
        console.log(`PP Armrest loaded: ${fetchedProducts.length}`);
        ...
        console.error('Error loading aremrest pp base:', error);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 max-w-2xl">
              Premium polypropylene armrest bases.
            </p>
          </div>
              ...
              {viewMode === 'grid' ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                  {filteredProducts.map((product, index) => (
                    <ProductCard 
                      ...
                      product={product} 
                      index={index}
                      categoryOverride="aremrest-pp-base"
                    />
                  ))}
                </div>
              ) : (
                <div className="space-y-3">
                  {filteredProducts.map((product, index) => (
                    ...
                    >
                      <Link href={`/products/aremrest-pp-base/${product.id}`}>
