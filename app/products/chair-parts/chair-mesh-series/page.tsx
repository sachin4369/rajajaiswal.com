...
import { fetchChairMeshSeries } from '@/lib/api';
import { Product } from '@/types';

export default function ChairMeshSeriesSubcategoryPage() {
  ...
  useEffect(() => {
    async function loadProducts() {
      ...
      setError(null);
      try {
        const fetchedProducts = await fetchChairMeshSeries();
        console.log(`Chair Mesh Series loaded: ${fetchedProducts.length}`);
        ...
        console.error('Error loading chair mesh series:', error);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <nav>
              <ol className="flex items-center space-x-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                ...
                <li>/</li>
                <li className="font-medium text-gray-900 dark:text-gray-100">Chair Mesh Series</li>
              </ol>
            </nav>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 max-w-2xl">
              Complete mesh series chair components.
            </p>
          </div>
              ...
              {viewMode === 'grid' ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                  {filteredProducts.map((product, index) => (
                    <ProductCard 
                      ...
                      product={product} 
                      index={index}
                      categoryOverride="chair-mesh-series"
                    />
                  ))}
                </div>
              ) : (
                <div className="space-y-3">
                  {filteredProducts.map((product, index) => (
                    ...
                    >
                      <Link href={`/products/chair-mesh-series/${product.id}`}>
